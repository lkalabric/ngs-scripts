...
function makeblast_db () {
	# Argumentos dentro da função:
    # $1 caminho de entrada dos dados INPUT_DIR
		INPUT_DIR=$1
	# $2 nome do banco de dados a ser criado
		NAME_DB=$(basename $INPUT_DIR)

	# Verifica se o diretório existe
	if [[ ! -d "$INPUT_DIR" ]]; then
	    echo "Erro: O diretório '$INPUT_DIR' não existe."
	    exit 1
	fi

	# =================================================================================
	# Script: makeblast_db.sh
	# Descrição: Cria o banco de dados BLAST a partir de arquivos .fasta
	...
	# Diretórios de dados
	# Salvar os arquivos contendo as sequências referências no formato Fasta (obtidos do Genbank) 
	...
	# Reseta o arquivo antes de concatenar novas sequências referências
	[[ -r ${REFSEQDIR}/refseq.fasta ]] && rm ${REFSEQDIR}/refseq.fasta
	
	# Diretório do banco de dados 
	BLASTDBDIR=${HOME}/data/BLAST_DB/${NAME_DB}
	...
	echo "Concatenando as sequências de referência em refseq.fasta..."
	if [[ ! -f ${INPUT_DIR}/refseq.fasta ]]
	then
		find ${INPUT_DIR} -type f -iname '*.fasta' -not -name 'refgen.fasta' -print0 | sort -z | xargs -0 cat > "${INPUT_DIR}/refseq.fasta"
	fi
	...
	mv ${INPUT_DIR}/refseq.fasta ${INPUT_DIR}/refseq.old
	...
	makeblastdb -in ${REFSEQDIR}/refseq.fasta -parse_seqids -blastdb_version 5 -taxid_map ${REFSEQDIR}/refseq.map -dbtype nucl -out ${BLASTDBDIR}
